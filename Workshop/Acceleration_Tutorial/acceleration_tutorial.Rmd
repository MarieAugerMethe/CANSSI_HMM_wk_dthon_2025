...
With these components specified, the HMM can be specified using `$fit(silent = TRUE)`.
...
#MAM: is there something to check convergence?
...
One of the advantage of the package `hmmTMB` is its capacity to easily incorporate complex and non-linear relationship with covariates. Here, `~s(TOD, bs="cc")` represents TOD using a cyclic spline.

#MAM: explain the bs = "cc"
...
#MAM: I don't see any difference with the previous obsvertion model. Can you explain why you need to create obs2?
...
We can compare the AIC scores using the `AIC` function for our original HMM (`hmm1`), the model with TOD as a covariate in the transition probabilities HMM (`hmm2`), and the model with TOD as a covariate in the state-dependent mean parameters HMM (`hmm3`).
