...
#MAM: I like the graph, but the representation of turning angle is slightly off. See https://www.researchgate.net/figure/Relationship-between-step-lengths-and-turning-angles-along-movement-path-recorded-for_tbl1_260084381 The way you have it what would be considered as not turning, would have an angle of 180.
...
```{r prep3, attr.source = ".numberLines"}
# calculate time of day variable (for later use)
tracks_gps$tod <- hour(tracks_gps$time) + minute(tracks_gps$time) / 60
```
...
```{r prep4, attr.source = ".numberLines"}
# calculate step length and turning angles
