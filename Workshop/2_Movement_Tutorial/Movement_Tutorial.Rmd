...
- Incorporating covariates in the observation model
...
of locations with large measurement error (e.g., you would
need to preprocess Argos data before applying the type of HMMs presented in this tutorial).

As is the case for the data of many aquatic species, the raw narwhal Fastloc-GPS data is collected at irregular time intervals (the time of locations is dictated partly with when they surface). To simplify the tutorial, we have already regularized the tracks. 
...
For some of the visualization, it's helpful to use a spatial object, and so we transform the track data into an `sf` object. The coordinate reference system of the original data is WGS84, which is defined by the EPSG code `4326`.

```{r, attr.source = ".numberLines"}
tracks_gps_sf <- tracks_gps %>% st_as_sf(coords = c("x", "y")) %>% 
  st_set_crs(4326)
```
...
knitr::include_graphics("example_ta_sl.png")
...
We can see that `prepData` has calculated the step length and the turning angle.  

# Fit HMM to data 

It is now possible to fit an HMM on this dataset using the R package `momentuHMM`.

## Fitting the model

Our data are ready to use, and we are almost ready to fit the HMM via the function `fitHMM`. 
We now need to make many important modelling decisions and most of these decisions will be associated with one of the arguments of the function. 
...
To start simple, we will only use two behavioural states. This means that the argument `nbStates` will be set to $2$.
...
It's useful to visualise your data in order to choose starting parameters.
...
Based on visual examination of the data, we might choose our starting parameters to be $0.1$ and $0.6$ for the mean step lengths and use half those values for the standard deviations. 
...
So now, lets fit the following model:

  * 2 states
  
  * Observations: Step length(gamma distribution) and turning angle (von Mises distribution)
...
Based on the mean step length parameters, it looks like the first behavioural state 
has smaller step lengths compared to state 2. The turning angle distributions of the second state indicates much more directed movement, with higher angular concentration.

Based on this, we may label state 1 as "resident" and state 2 as "travel". 


## Decoding the hidden sequence 

We are interested in identifying when an animal is in each of the behavioural states (i.e., when the animal is in state 1 vs state 2), what its called state decoding. 
...
As mentioned above, the starting parameter values can affect the estimating procedures. Thus, it's important to assess whether the parameter estimates are sensitive to the starting values we used (i.e., checking that we are not clearly at a local maximum). 
...
`momentuHMM` has functions to help selecting starting parameters for complex models, which you can explore in the Extra section.
...
Another covariate we will explore is time of day. Let's derive it.
...
We need to re-calculate data streams (step lengths and turning angles) using the `prepData` function, but this time include our covariates. 
For covariates that are part of the data frame with the location data, like time of day, we can use the argument `covNames` to distinguish these covariates from data streams. 
For spatial covariates in the form of rasters, like bathymetry, we will extract the bathymetry values at the location using the argument `spatialCovs`.
...
```{r prep4, attr.source = ".numberLines", cache=TRUE}
# calculate step length and turning angles
...
Here, we will will create a model where transition probabilities vary as a function of the bathymetry. Thus $\omega_1$ is the bathymetry value associated with that step and we can simply specify with a formula like the one used in simple linear models. 

```{r formula_bathy, cache = TRUE, attr.source = ".numberLines"}
formula_bathy <- ~bathy
```

As done earlier, we need to specify starting parameters and we will use the function `getPar0` to do so. This will use the estimated parameters of a previously fitted 2-state model with no covariates. 
...
# Here we first fit the model with no covariates: 
...
Par0_4 <- getPar0(model = mod3, formula = formula_bathy)
Par0_4
```

Then we can fit the model. The `formula` argument defines the model formula for the transition probabilities, and we need to set the correct starting parameters (i.e., `Par0` for the observation parameters and `beta0` for the transition probability model). 
...
Now we will illustrate how to model mean step length as a function of a  covariate. 
Any parameter in the observation model (e.g., mean or sd of the step lengths, or turning angle parameters) can be modelled as a function of covariates, which allows for the movement behaviour within each discrete state to vary. 
...
where 24 refers for the 24-hour period of interest. This relationship can conveniently be specified with the `cosinor` function. 
...
We can derive the starting parameters using the `getPar0` function, as described in the previous section.
...
# extract starting values from simpler 2-state model with no covariates
Par0_5 <- getPar0(model = mod3, DM = DM)
```
...
**Q6. Assess whether including time of day as a covariate in the transition probabilities improve the model fit. Interpret the results.** You need to read the extra section on covariates to be able to answer this question.

**Q7. Check if using time of day changes the pseudo-residuals.** You need to read the extra section on covariates to be able to answer this question.
...
## Polar bear exercise answers {#sec-res-pb}
...
There is plenty of persistence in turning angle (very high peak at 0) and though hard to exactly pick out there are maybe two different peaks in step length, something around 0.2 and 1.7.
