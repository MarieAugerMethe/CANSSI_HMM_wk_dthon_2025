...
head(fish_sim)
fish_sim <- fish_sim %>% filter(CommonName != "Mediterranean horse mackerel")
unique(fish_sim$CommonName)
write.csv(fish_sim, "fish_data.csv", row.names = FALSE)
```
...
```{r}
ggplot(data = fish_sim) +
  geom_line(aes(x = Year, y = Abd, colour = CommonName))
